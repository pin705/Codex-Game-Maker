...
2. **Playable Prototype Track**  
   贯穿 Pre-Production 到 Production，优先用 Godot 4.4 项目和导出预设验证 fun factor。
...
| Technical Setup -> Pre-Production | 是否选定 Godot 4.4 或已有引擎；是否有 asset import policy。 |
...
这个 demo 跑通后，再决定是否拆出 production/team workflow。这样能尽快验证 Codex + GPT Image 2 + Godot 4.4 的核心优势，而不是先陷入 72 个命令的机械搬运。
