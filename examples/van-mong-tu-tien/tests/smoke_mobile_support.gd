...
func _run() -> void:
	...
	var compact_snapshot: Dictionary = controls.get_layout_snapshot()
	_expect(float(compact_snapshot.get("joystick_ornament_diameter", 999.0)) <= joystick_rect.size.x * 0.68, "phone joystick art is compact inside its full touch target")
	_expect(float(compact_snapshot.get("pulse_ornament_diameter", 999.0)) <= pulse_rect.size.x * 0.72, "phone attack art is compact inside its full touch target")
