...
$recommendation = if ($detected.Count -eq 0) {
  if ($nestedProjects.Godot.Count -gt 0) {
    "No root engine project detected. Nested Godot project(s) found: $($nestedProjects.Godot -join ', '). Open one of those folders, or start a new root project with Godot 4.6.2 + Web export."
  } else {
    "Blank project: recommend Godot 4.6.2 + Web export."
  }
